...
test("test that you can see BMW M760 on view cars", async ({
  page,
}) => {
  await page.goto('https://course-user:Cou1dc4F@test.elitefleetgroup.engenious.io/');
...
test("test that you can see BMW M760 details", async ({
  page,
}) => {
  await page.goto('https://course-user:Cou1dc4F@test.elitefleetgroup.engenious.io/');
  await page.getByRole("link", { name: "View All Cars" }).click();
